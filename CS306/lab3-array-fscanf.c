...
//
// This version:
// -- Uses fscanf() (library I/O) to read from /proc/<PID>/io;
...
void server(int pfds[], char *filename)
{
  ...
  while ((message = readline(pfds[0])) != NULL) {
    ...
    ticks=time(NULL);
    timestr = asctime(localtime(&ticks));
...
void client(int pfds[])
{
  ...
  for (int i=1; i<=NUMBER_MESSAGES; i++) {
    ...
    fflush(procfp);  //Clear buffer so really re-read proc file contents.
 
    //Note:  had trouble with fscanf() call with fixed fields:
    //  fscanf(procfp,"rchar: %d \n wchar: %d\n",&rchar,&wchar)
    //Viewing fields from call used below showed wchar as wwchar on first call only.
    //This would cause match failure so only 1 field (rchar) was read on first call.
    if ((nread = fscanf(procfp,"%s %d %s %d\n",field,&rchar,field,&wchar))== EOF) {
      fprintf(stderr,"client %d: fscanf() failed: %s\n",pid,strerror(errno));
      exit(EXIT_FAILURE); }

    #ifdef DEBUG
    printf("fscanf num read: %d   rchar: %d    wchar: %d\n",nread,rchar,wchar);
    #endif
    ...
    snprintf(message,120,"%d: Message %d: read: %d  written: %d\n",pid,i,rchar,wchar);
